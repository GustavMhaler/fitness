...
function renderPlan() {
  ...
  $("#plan-grid").innerHTML = occurrences.map((occurrence, index) => {
    ...
    const isSelected = occurrence.plannedDate === app.activeDate;
    const isToday = occurrence.plannedDate === app.data.today;
    const label = occurrence.kind === "block" ? block?.name || "训练块" : occurrence.kind === "cardio" ? "有氧" : "休息";
    return `<button class="day-card ${isSelected ? "is-selected" : ""} ${isToday ? "is-today" : ""} ${occurrence.kind === "rest" ? "is-rest" : ""}" data-day="${occurrence.plannedDate}" aria-pressed="${isSelected}">
      <div class="flex items-center justify-between gap-2"><span class="text-xs font-semibold text-slate-500">DAY ${index + 1}</span><span class="flex items-center gap-2">${isToday ? `<span class="today-badge">今日</span>` : ""}${occurrence.status !== "planned" && occurrence.status !== "rest" ? `<span class="${statusClass(occurrence.status)}">${statusLabel(occurrence.status)}</span>` : ""}</span></div>
...
function renderToday() {
  ...
  const isToday = occurrence.plannedDate === app.data.today;
  const cardioPlanExerciseId = exerciseList[0]?.id || "cardio";
  const cardioActionPicker = exerciseList.length ? `<label class="field-label">动作<select name="actualExerciseId" class="compact-input mt-1">${exerciseList.map((exercise) => `<option value="${exercise.id}">${escapeHtml(exercise.name)}</option>`).join("")}</select></label>` : "";
  const stateText = session ? statusLabel(session.status) : occurrence.kind === "rest" ? "恢复日" : "准备开始";
  let detail = "";
  if (occurrence.kind === "rest") {
    detail = `<div class="mt-7 rounded-xl border border-white/5 bg-white/[0.025] p-5"><p class="text-sm font-semibold text-white">${isToday ? "今天不需要打卡" : "这一天不需要打卡"}</p><p class="mt-2 text-sm leading-6 text-slate-400">可以散步、拉伸，或者真正休息。休息日不会算作漏训。</p></div>`;
  } else if (occurrence.kind === "cardio") {
    detail = `<div class="mt-7 rounded-xl border border-cyan-300/10 bg-cyan-300/[0.04] p-5"><p class="text-sm font-semibold text-white">${escapeHtml(occurrence.cardioDesc || "自由有氧")}</p><p class="mt-2 text-sm text-slate-400">选择今天实际完成的有氧动作，填写时长或距离即可。</p><div class="mt-4 flex flex-wrap items-end gap-2"><form class="cardio-form flex flex-wrap items-end gap-2" data-date="${occurrence.plannedDate}" data-exercise-id="${cardioPlanExerciseId}">${cardioActionPicker}<label class="field-label">分钟<input name="duration" class="compact-input mt-1" inputmode="numeric" placeholder="时长" /></label><label class="field-label">公里<input name="distance" class="compact-input mt-1" inputmode="decimal" placeholder="距离" /></label><button class="secondary-button" type="submit">记录有氧</button></form><button class="primary-button" data-quick="${occurrence.plannedDate}">${session?.status === "completed" ? "已完成" : "快速完成"} <span>→</span></button><button class="ghost-button" data-skip="${occurrence.plannedDate}">跳过</button><button class="ghost-button" data-reschedule="${occurrence.plannedDate}">调休</button></div></div>`;
  } else {
  ...
  $("#today-card").innerHTML = `<div class="border-b border-white/5 p-5 sm:p-7"><div class="flex flex-wrap items-start justify-between gap-4"><div><p class="section-kicker">${isToday ? "今日" : "已选日期"}</p><h2 class="mt-2 text-2xl font-semibold text-white">${escapeHtml(occurrence.kind === "block" ? block?.name || "训练" : occurrence.kind === "cardio" ? block?.name || "有氧日" : "恢复日")}</h2><p class="mt-2 text-sm text-slate-400">${escapeHtml(formatDate(occurrence.plannedDate))} · ${escapeHtml(occurrence.kind === "block" ? block?.description || "" : occurrence.cardioDesc || "")}</p></div><span class="${statusClass(session?.status || occurrence.status)}">${stateText}</span></div>${session?.actualDate && session.actualDate !== session.plannedDate ? `<p class="mt-4 text-xs text-amber-200">实际训练日期：${escapeHtml(formatDate(session.actualDate))}</p>` : ""}</div><div class="p-5 sm:p-7">${detail}</div>`;
...
document.addEventListener("click", async (event) => {
  ...
  if (target.dataset.day) { app.activeDate = target.dataset.day; renderPlan(); return; }
